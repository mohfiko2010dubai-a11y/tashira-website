...
- Closed the remaining checkout-readiness UX blocker at `1cc8d12`. The application form now calls the authoritative readiness endpoint before opening payment UI, shows grouped missing requirements outside checkout, selects the first incomplete applicant through Complete Documents, and defers `loadStripe` until the ready-only payment component mounts. The server PaymentIntent gate remains unchanged.
- TypeScript, ESLint, 88/88 tests across 28 files, static-asset verification, and build passed. Exact-commit staging artifact deployment restarted only `tashira-staging`; production retained PID 875139. Browser UAT with an incomplete synthetic applicant showed the external completion panel, Passport copy and cover, Personal photo, and Complete Documents, with no Secure Payment UI and zero Stripe card frames. The staging API fixture continues to verify that a complete application may create a Stripe TEST intent.

- Closed the missing-document/payment-integrity blocker at `7a61e3e`: the PaymentIntent endpoint enforces a shared server readiness evaluation across application data, policy acceptance, valid pricing, every applicant, and conditional applicant-owned documents. The primary form uploads documents before checkout; incomplete applicants receive actionable per-applicant missing items.
- Consolidated payment completion into one Payment Confirmed state. Checkout unmounts after verified confirmation, canonical state is invalidated/refetched, paid refresh/back navigation cannot reopen payment, and the already-paid API conflict remains authoritative.
- Restored the canonical Node 20/npm 10 environment in an isolated staging-host clone with `npm ci`. TypeScript, ESLint, 85/85 tests across 27 files, static-asset verification, and the production build passed. Exact-commit artifact deployment restarted only `tashira-staging`; production retained PID 875139.
- Staging readiness UAT returned HTTP 412 for missing and partial documents, allowed a complete single applicant, blocked an incomplete second family applicant, and found no cross-application data disclosure. Stripe TEST replay UAT confirmed paid/succeeded state, matching amounts, one confirmation event, one invoice event, retry success, `requires_action` for 3DS, and replay-safe webhook handling. Interactive 3DS completion remains outstanding.
...
- Fixed the observed admin-page failure caused by an old browser bundle requesting a removed lazy chunk. Every lazy route now performs one guarded reload for recognized stale-chunk errors, while unrelated errors continue normally. Exact-commit staging verification passed TypeScript, ESLint, 71/71 tests across 23 files, build, HTTPS admin-page smoke, and production health checks.
