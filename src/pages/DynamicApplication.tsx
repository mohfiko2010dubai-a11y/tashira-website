...
export default function DynamicApplication() {
  ...
  return <main className="min-h-[70vh] bg-slate-50 px-4 py-8 sm:py-12">
    <div className="mx-auto max-w-3xl">
      ...
      {state.knownAnswers.length > 0 && <details className="mt-6 rounded-2xl border border-slate-200 bg-white p-5"><summary className="cursor-pointer font-semibold text-slate-900">Review previous answers</summary><ul className="mt-4 space-y-2 text-sm text-slate-700">{state.knownAnswers.map((item) => <li key={`${item.applicantId}-${item.code}`} className="flex justify-between gap-4 border-b border-slate-100 pb-2"><span>{item.code.replaceAll("_", " ")}</span><span className="font-medium">{String(item.answer)}</span></li>)}</ul></details>}
    </div>
  </main>;
}
