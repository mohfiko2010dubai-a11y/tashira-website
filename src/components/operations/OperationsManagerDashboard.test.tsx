...
const model: DashboardModel = { openCases: 3, waitingForCustomer: 1, scheduledSubmissions: 1, dueSoon: 1, urgent: 0,
  ...
  documentIntelligenceEscalations: 2, manualReviewApplicants: 1, manualReviewRatePercent: 12.5,
...
describe("Operations manager dashboard", () => {
  it("renders scoped operational metrics and supplier identity without finance data", () => {
    ...
    expect(html).toContain("Open cases"); expect(html).toContain("Document intelligence escalations"); expect(html).toContain("Manual review rate (%)");
    expect(html).toContain("Staff #7"); expect(html).toContain("Supplier #4");
