...
export default function VisaDeliveryPanel({ applicationId, applicationReference, applicants }: {
  ...
  return <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5" id="visa-delivery">
    ...
    <p className="mt-1 text-sm text-emerald-900">After the paid application reaches Visa Issued, upload the visa in the document panel, complete its required security review, then prepare it for the correct applicant. No other applicant can access it.</p>
    ...
    <button type="button" disabled={!applicantId || !documentId || visaReference.trim().length < 2 || validitySummary.trim().length < 3 || instructions.trim().length < 2 || prepare.isPending} onClick={async () => {
      setMessage(""); try { await prepare.mutateAsync({ applicationReference, applicantId: Number(applicantId), visaDocumentId: Number(documentId), visaReference: visaReference.trim(), validitySummary: validitySummary.trim(), customerInstructions: [instructions.trim()], commandId: crypto.randomUUID() }); setMessage("Visa approved for secure customer delivery."); } catch { setMessage("Visa delivery is blocked until the paid application is Visa Issued and ownership, permission and the latest security scan all pass."); }
