...
Status: **CANONICAL REUSE RUNBOOK — NOT AN AUTHORIZATION**. Migrations `014–042` were already applied to isolated Staging during separately authorized, backed-up phases. Migration `043` adds the canonical missing RBAC permission codes and must be rehearsed before its separately authorized Staging application. This document does not authorize a new staging connection, migration, deployment, rollback or feature activation.
...
- Run `node --experimental-strip-types scripts/verify-operations-production-readiness.ts` from the exact clean review branch and confirm migrations `014` through `043` match the reviewed manifest.
...
30. `043_operations_permission_catalog.sql`
...
for migration in \
  ...
  migrations/043_operations_permission_catalog.sql
do
...
- Verify every schema object and catalog row required by `014–043` and compare against the isolated round-trip rehearsal evidence.
...
2. any future Staging replay or recovery involving migrations `014–043`, with all features OFF;
...
For a fresh/recovered Staging database only, apply `014–043` to proven `tashira_staging`, verify integrity, keep all flags OFF, then stop for review. On the current Staging database, verify migrations `014–042`, create and verify a new backup, then apply only pending Migration `043`; never blindly reapply historical migrations.
