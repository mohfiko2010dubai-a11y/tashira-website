...
function json<T>(row: object,key:string):T { const item=value(row,key); return (typeof item==="string"?JSON.parse(item):item) as T; }
...
export class MysqlSupplierSlaRepository {
  ...
  async apply(slaId:string,command:SupplierSlaCommand):Promise<SupplierSlaResource>{const connection=await this.pool.getConnection();try{
    ...
    if(replay[0]){const metadata=json<{command:{action:string;expectedVersion:number;actorStaffId:number;reason:string};result:SupplierSlaResource}>(replay[0],"metadata");const comparable={action:command.action,expectedVersion:command.expectedVersion,actorStaffId:command.actorStaffId,reason:command.reason};if(JSON.stringify(metadata.command)!==JSON.stringify(comparable))throw new Error("SUPPLIER_SLA_IDEMPOTENCY_CONFLICT");await connection.commit();return metadata.result;}
