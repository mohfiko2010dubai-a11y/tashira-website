...
describe("Operations Staging runbook release alignment", () => {
  ...
  it("requires the full current migration chain and release manifest gate", () => {
    ...
    expect(source).toContain("migrations/041_visa_rule_lifecycle_evidence.sql");
  ...
  it.each(["TYPING_PACK", "AUTHORITY_QUERY", "VISA_DELIVERY", "REGULATORY_WATCHER", "OPERATIONS_EMAIL_AUTOMATION"])("keeps %s in the explicit closed-flag inventory", (flag) => expect(source).toContain(`\`${flag}\``));
});
